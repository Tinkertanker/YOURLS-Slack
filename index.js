...
app.post("/shorten", function(req, res) {
  ...
  if (!args[0] || !args[1]) {
    res.send(
      JSON.stringify({ response_type: "in_channel", text: "Missing Arguments" })
    );
    return;
  }
  res.send(
    JSON.stringify({
      ...
      text: "Verified. Attemping to shorten! :link:"
    })
  );
  fetch(
    `${config.yourl}/yourls-api.php?signature=${encodeURI(
      config.signature
    )}&action=shorturl&format=json&url=${encodeURI(
      args[0]
    )}&keyword='${encodeURI(args[1].toLowerCase())}'`
  )
...
app.post("/unshorten", function(req, res) {
  ...
  res.send(
    JSON.stringify({
      response_type: "in_channel",
      text: "Verified. Attemping to unshorten! :boom:"
    })
  );
  fetch(
    `${config.yourl}/yourls-api.php?signature=${encodeURI(
    ...
    .then(json => {
      if (json.statusCode != 200) {
        fetch(req.body.response_url, {
          ...
          body: JSON.stringify({
            response_type: "in_channel",
            text: `${json.message.charAt(0).toUpperCase() +
              json.message.slice(1)}`
          }),
          headers: { "Content-Type": "application/json" }
        });
      }
